...
public class Feeder extends SubsystemBase {
  ...
  private final FlyWheelConfig feederConfig = new FlyWheelConfig(motor)
  ...
  private void updateInputs() {
    ...
    feederInputs.setpoint = motor.getMechanismSetpointVelocity().orElse(RPM.of(0));
    feederInputs.volts = motor.getVoltage();
    feederInputs.current = motor.getStatorCurrent();
  }
  ...
  public Command setVelocity(Supplier<AngularVelocity> speed) {
    return feeder.setSpeed(() -> {
      Logger.recordOutput("Feeder/Setpoint",
          speed.get());
      return speed.get();
    });
  }
  ...
  public Command setDutyCycle(Supplier<Double> dutyCycle) {
    return feeder.set(() -> {
      Logger.recordOutput("Feeder/DutyCycle",
          dutyCycle.get());
